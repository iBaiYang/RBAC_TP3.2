	...
	<head>
		...
		<title>开联通互联网支付平台-商户接口范例-单笔订单查询确认</title>
	...
	?>
		...
		<!-- 1. 页面方式提交查询请求 -->
		<div>
			<form name="form1" action="<?='http://'+$serverIP+$serverUrl?>" method="post">
    ...
    $pageContents = "";
		...
		if($map->get("ERRORCODE") != ""){
			...
			if($map->get("issuerId") != "")
			$bufVerifySrc = $bufVerifySrc."issuerId=".($map->get("issuerId"))."&";		//issuerId
				
			if($map->get("mchtOrderId") != "")
			$bufVerifySrc = $bufVerifySrc."mchtOrderId=".($map->get("mchtOrderId"))."&";		//mchtOrderId
			...
			$verifyMsg = urldecode($map->get("signMsg"));

			//取交易结果量
			$payResult = $map->get("payResult");		//交易状态 payResult=1表示交易成功
