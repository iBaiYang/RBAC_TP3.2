...
Class Request{
    ...
    public function __construct($notify_url, $callback_url){
        $this->Request();
    ...
    public function submitOrderInfo(){
        ...
        //通知地址，必填项
		$this->reqHandler->setParameter('notify_url', $this->notify_url);//通知回调地址，目前默认是空格，商户在测试支付和上线时必须改为自己的，且保证外网能访问到
		$this->reqHandler->setParameter('callback_url', $this->callback_url);
    ...
    /**
     * 提供给威富通的回调方法
     * @param object $object 数据库实例
     * @param string $fun    执行实例中的方法名
     */
    public function callback($object, $fun){
        ...
        if($this->resHandler->isTenpaySign()){
            if($this->resHandler->getParameter('status') == 0 && $this->resHandler->getParameter('result_code') == 0){
                ...
                // 执行回调
                call_user_func(array($object, $fun), $this->resHandler->getAllParameters());
                exit();
            }else{
                echo 'failure';
                exit();
